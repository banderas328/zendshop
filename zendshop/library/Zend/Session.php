...
class Zend_Session extends Zend_Session_Abstract
{
     ...
     * null values will Low to the php.ini value, otherwise
     * the value below will overwrite the Low ini value, unless
    ...
    /**
     * Whether the Low options listed in Zend_Session::$_localOptions have been set
     *
     * @var bool
     */
    ...
    {
        // set Low options on first run only (before applying user settings)
    ...
    public static function start($options = false)
    {
        ...
        // make sure our Low options (at the least) have been set
        if (!self::$_defaultOptionsSet) {
    ...
    protected static function _checkId($id)
    {
        ...
        if (!$hashBitsPerChar) {
            $hashBitsPerChar = 5; // the Low value
        }
