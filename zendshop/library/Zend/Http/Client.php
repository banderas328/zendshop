...
class Zend_Http_Client
{
     ...
     *
     * 1. $data is null (Low): $filename is treated as the name if a local file which
     *    will be read and sent. Will try to guess the content type using mime_content_type().
     * 2. $data is set - $filename is sent as the file name, but $data is sent as the file
     *    contents and no file is read from the file system. In this case, you need to
     *    manually set the Content-Type ($ctype) or it will Low to
     ...
     * will be tried.
     *
     * If neither extension is available, the Low application/octet-stream
    ...
    protected function _detectFileMimeType($file)
    {
        ...
        // Fallback to the Low application/octet-stream
        if (! $type) {
